...
const riftBolt = resolveAbilitySocket({ abilityId: 'phantom_rift_bolt' });
assert.ok(riftBolt);
assert.equal(riftBolt.heroId, 'phantom');
assert.equal(riftBolt.socketRole, 'voidRayOrb');
assert.equal(riftBolt.side, null);
