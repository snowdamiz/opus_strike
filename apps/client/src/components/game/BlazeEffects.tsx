...
function pushBurningPlayerId(target: string[], player: Player | null | undefined, now: number): void {
  if (!shouldShowBurningPlayer(player, now)) return;
  if (target.includes(player.id)) return;
  target.push(player.id);
}
...
function collectBurningPlayerIds(target: string[]): string[] {
  ...
  for (const player of state.players.values()) {
    pushBurningPlayerId(target, player, now);
  }
  pushBurningPlayerId(target, state.localPlayer, now);
...
export function BlazeEffectsManager() {
  ...
  useFrame((_, delta) => {
    ...
    if (scanAccumulatorRef.current < BLAZE_EFFECT_SCAN_INTERVAL_MS) return;
